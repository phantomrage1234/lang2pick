...
so101_control::so101_hardware_interface::on_init(
  ...
  num_joints_ = params.joints.size();
  std::unordered_set<std::string> joint_names, state_interface_names,
  ...
  for (int i = 0; i < num_joints_; ++i) {
    ...
    for (const auto &state_interface : params.joints[i].state_interfaces) {
      if (state_interface_names.find(state_interface.name) !=
        ...
        RCLCPP_FATAL(rclcpp::get_logger("so101_hardware_interface"),
                     "Joint %s has duplicate/unknown state interface %s.",
                     params.joints[i].name.c_str(),
    ...
    servos_[i].cmd_id =
        params.joints[i].parameters.at("type") == "position" ? 0.0 : 1.0;
  }
...
so101_control::so101_hardware_interface::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_INFO(rclcpp::get_logger("so101_hardware_interface"),
              "Activating hardware interface for %d joints.", num_joints_);
...
so101_control::so101_hardware_interface::update_hardware_status(int id) {
  ...
  if (motor_driver_.FeedBack(id) == -1) {
    ...
    servos_[id - 1].position_state =
        motor_driver_.ReadPos(id) * 2 * M_PI / steps_per_rev_;
    servos_[id - 1].velocity_state =
        motor_driver_.ReadSpeed(id) * 2 * M_PI / steps_per_rev_;
...
so101_control::so101_hardware_interface::export_state_interfaces() {
  ...
  for (int i = 0; i < num_joints_; ++i) {
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        info_.joints[i].name, "position", &servos_[i].position_state));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        info_.joints[i].name, "velocity", &servos_[i].velocity_state));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        info_.joints[i].name, "torque", &servos_[i].torque_state));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        info_.joints[i].name, "temperature", &servos_[i].temperature_state));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
        info_.joints[i].name, "voltage", &servos_[i].voltage_state));
  }
...
hardware_interface::return_type so101_control::so101_hardware_interface::read(
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
  for (int i = 1; i <= num_joints_; ++i) {
    if (update_hardware_status(i) ==
...
hardware_interface::return_type so101_control::so101_hardware_interface::write(
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) {
  for (int i = 0; i < num_joints_; ++i) {
    int id = servos_[i].id;
